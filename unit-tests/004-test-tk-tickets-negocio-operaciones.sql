-- Pruebas de espera, cancelación y asociación de activos
...
BEGIN
    ...
    TICKETS.TK_TICKETS_NEGOCIO_PKG.PONER_EN_ESPERA(
        v_id_ticket, v_id_supervisor, 'Esperando información del solicitante.',
    ...
    TICKETS.TK_TICKETS_NEGOCIO_PKG.CANCELAR_TICKET(
        v_id_ticket, v_id_supervisor, 'Cancelación solicitada para la prueba.',
        v_id_comentario, v_exito, v_codigo, v_mensaje
    );
    ...
    -- Crear un activo temporal para probar la relación activo-ticket.
    ...
    TICKETS.TK_TICKETS_NEGOCIO_PKG.CREAR_TICKET(
        'TEST ACTIVO ' || v_marca_tiempo, TO_CLOB('Prueba de asociación de activo.'),
    ...
    afirmar(v_count = 1, 'validar relación activo-ticket');
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        END IF;
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('=== PRUEBA DE OPERACIONES FALLÓ ===');
        RAISE;
END;
/
